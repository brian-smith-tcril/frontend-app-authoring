...
const deriveHandlerUrl = async ({
  ...
  if (isLibrary) {
    return client
      .get(urls.boundHandlerUrl({ studioEndpointUrl, blockId, handlerName }))
      .then((response: AxiosResponse<{ handler_url: string }>) => response.data.handler_url);
  }
...
export const useBlockHandlerData = <T>({ blockId, uniqueId, handlerName, defaultData }: UseBlockDataParams<T>) => {
